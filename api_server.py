...
<head>
    ...
    <style>
        * {{ margin: 0; padding: 0; box-sizing: border-box; }}
        ...
        #confirmBtn {{ background: #27ae60; color: white; border: none; padding: 18px 40px; font-size: 20px; font-weight: bold; border-radius: 50px; cursor: pointer; margin-top: 20px; }}
        #confirmBtn:hover {{ background: #229954; }}
...
<body>
    <div class="container">
        ...
        <p>1. اضغط على "فتح الإعلان"<br>2. شاهد الإعلان كاملاً<br>3. عد واضغط "أكد المشاهدة"</p>
        <a href="{AD_LINK}" target="_blank" class="ad-link" onclick="adOpened=true">🌐 فتح الإعلان</a>
        <br><br>
        <button id="confirmBtn" onclick="confirmView()">✅ أكد المشاهدة</button>
        <div id="message"></div>
    </div>
    <script>
        let adOpened = false;
        async function confirmView() {{
            ...
            if (!adOpened) {{ msgDiv.className = 'error'; msgDiv.style.display = 'block'; msgDiv.innerHTML = '⚠️ افتح الإعلان أولاً!'; return; }}
            btn.disabled = true; btn.textContent = '⏳ جاري التحقق...';
            try {{
                const res = await fetch('/api/complete-ad', {{ method: 'POST', headers: {{ 'Content-Type': 'application/json' }}, body: JSON.stringify({{ token: '{token}' }}) }});
                const data = await res.json();
                msgDiv.className = data.success ? 'success' : 'error';
                msgDiv.innerHTML = data.success ? '✅ تم التحقق!<br>يمكنك العودة للبوت' : '❌ خطأ';
                msgDiv.style.display = 'block';
            }} catch {{ msgDiv.className = 'error'; msgDiv.innerHTML = '❌ خطأ في الاتصال'; msgDiv.style.display = 'block'; btn.disabled = false; btn.textContent = '✅ أكد المشاهدة'; }}
        }}
    </script>
</body>
</html>"""
...
<body>
    <div class="container">
        ...
        <p>1. اضغط "فتح المهمة"<br>2. أكمل المطلوب<br>3. عد واضغط "أكد الإكمال"</p>
        <a href="{task.get('task_url', '#')}" target="_blank" class="task-link" onclick="taskOpened=true">🔗 فتح المهمة</a>
        <br><br>
        <button id="confirmBtn" onclick="confirmTask()">✅ أكد الإكمال</button>
        <div id="message"></div>
    </div>
    <script>
        ...
        async function confirmTask() {{
            const btn = document.getElementById('confirmBtn');
            const msgDiv = document.getElementById('message');
            if (!taskOpened) {{ msgDiv.className = 'error'; msgDiv.style.display = 'block'; msgDiv.innerHTML = '⚠️ افتح المهمة أولاً!'; return; }}
            btn.disabled = true; btn.textContent = '⏳ جاري التحقق...';
            try {{
                const res = await fetch('/api/complete-task', {{ method: 'POST', headers: {{ 'Content-Type': 'application/json' }}, body: JSON.stringify({{ token: '{token}' }}) }});
                const data = await res.json();
                msgDiv.className = data.success ? 'success' : 'error';
                msgDiv.innerHTML = data.success ? '✅ تم التحقق!<br>يمكنك العودة للبوت' : '❌ خطأ';
                msgDiv.style.display = 'block';
            }} catch {{ msgDiv.className = 'error'; msgDiv.innerHTML = '❌ خطأ في الاتصال'; msgDiv.style.display = 'block'; btn.disabled = false; btn.textContent = '✅ أكد الإكمال'; }}
        }}
    </script>
</body>
</html>"""
